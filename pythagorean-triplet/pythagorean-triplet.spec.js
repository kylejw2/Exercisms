...
describe('Triplet', () => {
  ...
  test('can make triplets up to 10', () => {
    const triplets = Triplet.where({ maxFactor: 10 }); // c must be less than 11
  ...
  test('can make triplets 11 through 20', () => {
    const triplets = Triplet.where({ minFactor: 11, maxFactor: 20 }); // a must be greater than 10 and c must be less than 21
    const products = triplets.sort().map(triplet => triplet.product());
    expect(products).toEqual([3840]);
  });

  test('can filter on sum', () => {
    const triplets = Triplet.where({ sum: 180, maxFactor: 100 }); // c must be less than 101 and a + b + c = 180
    const products = triplets.sort().map(triplet => triplet.product());
